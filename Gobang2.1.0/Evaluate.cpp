...
void Game::Search(int row, int col, int chessFlag)
{
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 5; j++)
		{
			poinT point = DirectionCount_New(row, col, -count[i].left - j - 1, i);
			if (point.x < 0 || point.y>14)
				left[j] = 0;
			else
				left[j] = chess[point.x][point.y].flag;
			point = DirectionCount_New(row, col, count[i].right + j + 1, i);
			if (point.x < 0 || point.y>14)
				right[j] = 0;
			else
				right[j] = chess[point.x][point.y].flag;
		}
...
{
	if (left[0] == SPACE && left[1] == SPACE && right[0] == SPACE && right[1] == SPACE);
	else
	{
